    ...
    for (i = 0; i < NUM_INV_LISTS; i++) {
        ...
        if (inv_notebooks[i].type == INV_TREE) {
                    ...
                    GTK_POLICY_NEVER, GTK_POLICY_ALWAYS);
            {
                GdkPixbuf *pb;
#ifdef WIN32
                /* On Windows the XPM loader plugin is not available in the
                 * bundled GTK runtime. Use pre-converted inline RGBA data
                 * generated by gdk-pixbuf-csource instead. */
                static const struct { const char *name; const guint8 *data; } inline_map[] = {
                    {"all",          all_inline},
                    {"applied",      hand_inline},
                    {"unapplied",    hand2_inline},
                    {"unpaid",       coin_inline},
                    {"cursed",       skull_inline},
                    {"magical",      mag_inline},
                    {"nonmagical",   nonmag_inline},
                    {"locked",       lock_inline},
                    {"unlocked",     unlock_inline},
                    {"unidentified", unidentified_inline},
                };
                pb = NULL;
                for (int j = 0; j < (int)(sizeof(inline_map)/sizeof(inline_map[0])); j++) {
                    if (strcmp(inv_notebooks[i].name, inline_map[j].name) == 0) {
                        pb = gdk_pixbuf_new_from_inline(-1, inline_map[j].data, FALSE, NULL);
                        break;
                    }
                }
#else
                pb = gdk_pixbuf_new_from_xpm_data((const char**) inv_notebooks[i].xpm);
#endif
